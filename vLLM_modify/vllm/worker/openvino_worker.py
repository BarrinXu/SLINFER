...
class OpenVINOWorker(LoraNotSupportedWorkerBase):
    ...
    def process_layer(self, layer, k_cache, v_cache, physical_block_list, final_tensor):
        for i, block_idx in enumerate(physical_block_list):
            final_tensor[layer, 0, i] = k_cache[block_idx]
        for i, block_idx in enumerate(physical_block_list):
            final_tensor[layer, 1, i] = v_cache[block_idx]
    ...
    def save_kv_cache_chxu(self, seq_group_metadata_list: List[SequenceGroupMetadata]):
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            for seq_id, physical_block_list in seq_group_metadata.block_tables.items():
                ...
                physical_block_index_tensor = torch.tensor(physical_block_list)
                layers_kv_blocks = []
                layer_num = len(kv_caches)
                block_num = len(physical_block_list)
                tensor_shape: list = list(kv_caches[0][0].get_shape())
                tensor_shape = [layer_num, 2, block_num] + tensor_shape[1:]
                logger.warning(f'       tensor_shape: {tensor_shape}')
                final_tensor = torch.empty(tensor_shape, dtype=torch.float16)
                final_tensor.share_memory_()
                for layer, kv_cache in enumerate(kv_caches):
                    logger.warning(f'           Save kv_cache for layer {layer}')
                    k_cache = kv_cache[0]
                    v_cache = kv_cache[1]
                    k_cache_np = k_cache.data
                    k_cache_tensor = torch.from_numpy(k_cache_np)
                    for i, block_idx in enumerate(physical_block_list):
                        final_tensor[layer, 0, i] = k_cache_tensor[block_idx]
                    v_cache_np = v_cache.data
                    v_cache_tensor = torch.from_numpy(v_cache_np)
                    for i, block_idx in enumerate(physical_block_list):
                        final_tensor[layer, 1, i] = v_cache_tensor[block_idx]
    ...
    def restore_kv_cache_chxu(self, execute_model_req: ExecuteModelRequest) -> None:
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            if seq_data.kv_cache_need_restore is True:
                # # do something
                # kv_info = kv_manager.pop_request(seq_group_metadata.request_id)
                # data_config = kv_info.data_config
                # src_kv_tensor = kv_info.tensor
                # src_kv_np = src_kv_tensor.numpy()
                # src_kv_tensor_shape = src_kv_tensor.shape
                # request_id = data_config['request_id']
                # device = data_config['device']
                # assert device == 'cpu'
                # for seq_id, physical_block_list in seq_group_metadata.block_tables.items():
                #     logger.warning(f'       src_kv_tensor_shape: {src_kv_tensor_shape}')
                #     logger.warning(f'       Handle request_id {request_id}, '
                #                    f'physical_blocks_num: {len(physical_block_list)}, '
                #                    f'physical_blocks: {physical_block_list}')
                #
                #     src_layer_num = src_kv_tensor_shape[0]
                #     src_block_num = src_kv_tensor_shape[2]
                #     assert src_layer_num == len(kv_cache)
                #     assert src_kv_tensor_shape[1] == 2
                #     assert src_block_num <= len(physical_block_list)
                #     for layer, target_kv_cache in enumerate(kv_cache):
                #         if device == 'gpu':
                #             pass
                #             # restored_kv_cache_tensor = (restored_kv_cache_tensor.to(torch.bfloat16).
                #             #                             view(torch.float16).permute(0, 1, 3, 2, 4))
                #         assert list(src_kv_tensor_shape[3:]) == list(target_kv_cache[0].shape[1:])
                #         for i in range(2):
                #             target_ov_tensor: ov.Tensor = target_kv_cache[i]
                #             target_np_tensor: np.ndarray = target_ov_tensor.data
                #             for src_index in range(src_block_num):
                #                 target_np_tensor[physical_block_list[src_index]] = src_kv_np[layer][i][src_index]
                #     logger.warning(f'       Restore-kv request_id {request_id} complete')
